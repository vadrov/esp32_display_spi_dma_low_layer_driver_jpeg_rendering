...
//типы ползунков
typedef enum {
	MGL_SLIDER_HORIZONTAL = 0,		//горизонтальный
	MGL_SLIDER_VERTICAL				//вертикальный
} MGL_SLIDER_TYPES;

//данные ползунка
typedef struct {
	int x1, y1, x2, y2;				//координаты вершин
	MGL_SLIDER_TYPES type;			//тип ползунка: горизонтальный или вертикальный
	int value_min, value_max, value;//минимальное, максимальное и текущее значение параметра
	char *unit;						//указатель на строку с единицей измерения (метры, градусы, вольты и т.п.)
	MGL_OBJ *obj_fon;				//указатель на объект - прямоугольник, определяющий фон элементов ползунка
	MGL_OBJ *obj_rectangle1;		//указатель на объект - прямоугольник, определяющий полосу прокрутки ползунка
	MGL_OBJ *obj_rectangle2;		//указатель на объект - прямоугольник, определяющий полосу прокрутки ползунка
	MGL_OBJ *obj_circle; 			//указатель на объект - круг, определяющий ползунок
	MGL_OBJ *obj_text;				//указатель на объект - текст надписи, содержащей параметр с единицей измерения
	uint32_t color;					//цвет фона
} MGL_OBJ_SLIDER;
...
//устанавливает параметры ползунка
